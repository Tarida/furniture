...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            signup() {
                this.performingRequest = true
                let newusr = new Promise((resolve,reject)=>{
                    fb.auth.createUserWithEmailAndPassword(this.signupForm.email, this.signupForm.password).then(userc => {
                        resolve(userc)
                    }).catch(err => {
                        reject(err)
                    })
                ...
                newusr.then(usr=>{
                    console.log("Running firestore")
                    this.$store.commit('setCurrentUser', usr.user)
                    fb.usersCollection.doc(usr.user.uid).set({
